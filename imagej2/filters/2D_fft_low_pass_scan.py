...
def fft_low_pass_filter(fft_array, radius):
    """
    Apply a low pass filter to an FFT frequency array.

    :param fft_array: An FFT frequency array/image.
    :param radius: Radius of the quarter circle low pass filter.
    """
    # get cursor
...
def fft_scan(img, start_radius, end_radius):
    ...
    for radius in range(start_radius, end_radius):
        ...
        fft_low_pass_filter(fft_array_dup, radius)
